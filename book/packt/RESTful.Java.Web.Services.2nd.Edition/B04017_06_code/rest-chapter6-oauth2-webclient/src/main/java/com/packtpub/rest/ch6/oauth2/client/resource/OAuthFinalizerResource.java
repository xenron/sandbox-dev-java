 ...
 * 
 * User will be redirected back to this resource after he/she grants access to
 * our application.
 *
 * @author Jobinesh
 */
@Path("oauth2")
public class OAuthFinalizerResource {

    @Context
    private UriInfo uriInfo;

    /**
     * This method will be called by the Authorization Server using redirect URI if
     * resource owner approved access to this application.
     *
     * @param code
     * @param state
     * @return
    ...
    public Response authorize(@QueryParam("code") String code, @QueryParam("state") String state) {
        //Get the cached instnace of OAuth2CodeGrantFlow via OAuthServiceContext
        OAuthServiceContext oauthServiceContext = OAuthServiceContext.getInstnace(null);
         OAuth2CodeGrantFlow flow = oauthServiceContext.getFlow();

        //Finish the authorization process and return the TokenResult
         TokenResult tokenResult = flow.finish(code, state);

        oauthServiceContext.setAccessToken(tokenResult.getAccessToken());

        // Authorization is finished,now redirect user back to the task resource

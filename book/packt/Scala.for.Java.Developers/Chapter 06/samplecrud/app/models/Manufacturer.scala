...
case class ManufacturerRow(manufacturerId : Option[Int], name : Option[String], addressline1 : Option[String], addressline2 : Option[String], city : Option[String], state : Option[String], zip : Option[String], phone : Option[String], fax : Option[String], email : Option[String], rep : Option[String])

// Definition of the ManufacturerRow table
object Manufacturer extends Table[ManufacturerRow]("MANUFACTURER") {
  ...
  def * = manufacturerId.? ~ name ~ addressline1 ~ addressline2 ~ city ~ state ~ zip ~ phone ~ fax ~ email ~ rep <> (ManufacturerRow.apply _, ManufacturerRow.unapply _)
  //def autoInc = id.? ~ manufacturerId.? ~ name ~ addressline1 ~ addressline2 ~ city ~ state ~ zip ~ phone ~ fax ~ email ~ rep <> (ManufacturerRow.apply _, ManufacturerRow.unapply _) returning id 
  ...
  def list(page: Int = 0, pageSize: Int = 10, orderBy: Int = 1, order: String, asc: Boolean = false, filter: String = "%") = {
    val members = ru.typeOf[ManufacturerRow].members.filter(m => m.isTerm && !m.isMethod).toList

...
case class ProductCodeRow(prodCode : Option[String], discountCode : String, description : Option[String])

// Definition of the ProductCodeRow table
object ProductCode extends Table[ProductCodeRow]("PRODUCT_CODE") {
  ...
  def * = prodCode.? ~ discountCode ~ description <> (ProductCodeRow.apply _, ProductCodeRow.unapply _)
  //def autoInc = id.? ~ prodCode.? ~ discountCode ~ description <> (ProductCodeRow.apply _, ProductCodeRow.unapply _) returning id 
  ...
  def list(page: Int = 0, pageSize: Int = 10, orderBy: Int = 1, order: String, asc: Boolean = false, filter: String = "%") = {
    val members = ru.typeOf[ProductCodeRow].members.filter(m => m.isTerm && !m.isMethod).toList

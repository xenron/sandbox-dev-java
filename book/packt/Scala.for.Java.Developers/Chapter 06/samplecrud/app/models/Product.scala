...
case class ProductRow(productId : Option[Int], manufacturerId : Int, productCode : String, purchaseCost : Option[Int], quantityOnHand : Option[Int], markup : Option[Int], available : Option[String], description : Option[String])

// Definition of the ProductRow table
object Product extends Table[ProductRow]("PRODUCT") {
  ...
  def * = productId.? ~ manufacturerId ~ productCode ~ purchaseCost ~ quantityOnHand ~ markup ~ available ~ description <> (ProductRow.apply _, ProductRow.unapply _)
  //def autoInc = id.? ~ productId.? ~ manufacturerId ~ productCode ~ purchaseCost ~ quantityOnHand ~ markup ~ available ~ description <> (ProductRow.apply _, ProductRow.unapply _) returning id 
  ...
  def list(page: Int = 0, pageSize: Int = 10, orderBy: Int = 1, order: String, asc: Boolean = false, filter: String = "%") = {
    val members = ru.typeOf[ProductRow].members.filter(m => m.isTerm && !m.isMethod).toList

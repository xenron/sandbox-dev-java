...
case class DiscountCodeRow(discountCode : Option[String], rate : Option[Int])

// Definition of the DiscountCodeRow table
object DiscountCode extends Table[DiscountCodeRow]("DISCOUNT_CODE") {
  ...
  def * = discountCode.? ~ rate <> (DiscountCodeRow.apply _, DiscountCodeRow.unapply _)
  //def autoInc = id.? ~ discountCode.? ~ rate <> (DiscountCodeRow.apply _, DiscountCodeRow.unapply _) returning id 
  ...
  def list(page: Int = 0, pageSize: Int = 10, orderBy: Int = 1, order: String, asc: Boolean = false, filter: String = "%") = {
    val members = ru.typeOf[DiscountCodeRow].members.filter(m => m.isTerm && !m.isMethod).toList

...
case class CustomerRow(customerId : Option[Int], discountCode : String, zip : String, name : Option[String], addressline1 : Option[String], addressline2 : Option[String], city : Option[String], state : Option[String], phone : Option[String], fax : Option[String], email : Option[String], creditLimit : Option[Int])

// Definition of the CustomerRow table
object Customer extends Table[CustomerRow]("CUSTOMER") {
  ...
  def * = customerId.? ~ discountCode ~ zip ~ name ~ addressline1 ~ addressline2 ~ city ~ state ~ phone ~ fax ~ email ~ creditLimit <> (CustomerRow.apply _, CustomerRow.unapply _)
  //def autoInc = id.? ~ customerId.? ~ discountCode ~ zip ~ name ~ addressline1 ~ addressline2 ~ city ~ state ~ phone ~ fax ~ email ~ creditLimit <> (CustomerRow.apply _, CustomerRow.unapply _) returning id 
  ...
  def list(page: Int = 0, pageSize: Int = 10, orderBy: Int = 1, order: String, asc: Boolean = false, filter: String = "%") = {
    val members = ru.typeOf[CustomerRow].members.filter(m => m.isTerm && !m.isMethod).toList

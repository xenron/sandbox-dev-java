...
object ProductController extends Controller { 
  ...
  val form = Form(
    mapping(
      ...
      "purchaseCost" -> optional(number),
      "quantityOnHand" -> optional(number),
      "markup" -> optional(number),
      "available" -> optional(nonEmptyText),
      "description" -> optional(nonEmptyText)
    )(ProductRow.apply)(ProductRow.unapply)
  )
  ...
  /**
   * Display the paginated list of product.
   *
  ...
  def list(page: Int, orderBy: Int, filter: String = "%") = Action { implicit request =>
    database withSession {
      Ok(html.product.list(
        Page(Product.list(page, pageSize, orderBy, filter).list,
        page, 
        offset = pageSize * page,
        Product.findAll(filter).list.size),
        orderBy,
        filter))
    }
  }
  ...
  def create = Action {
    database withSession {
      Ok(html.product.createForm(form)) // ,supplierSelect))
    }
  }
  ...
  def save = Action { implicit request =>
    form.bindFromRequest.fold(
      formWithErrors => BadRequest(html.product.createForm(formWithErrors)), // supplierSelect)),
      entity => {
  ...
  def edit(pk: Int) = Action {
    database withSession {
      Product.findByPK(pk).list.headOption match {
        case Some(e) => Ok(html.product.editForm(pk, form.fill(e))) //, supplierSelect))
        case None => NotFound
      }
    }
  }
  ...
  def update(pk: Int) = Action { implicit request =>
    database withSession {
      form.bindFromRequest.fold(
        formWithErrors => BadRequest(html.product.editForm(pk, formWithErrors)), // supplierSelect)),
        entity => {
          Home.flashing(Product.findByPK(pk).update(entity) match {
  ...
  def delete(pk: Int) = Action {
    database withSession {
      Home.flashing(Product.findByPK(pk).delete match {

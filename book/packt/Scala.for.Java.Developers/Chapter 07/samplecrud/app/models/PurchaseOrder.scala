...
case class PurchaseOrde(orderNum : Option[Int], customerId : Int, productId : Int, quantity : Option[Int], shippingCost : Option[Int], salesDate : Option[Date], shippingDate : Option[Date], freightCompany : Option[String])

// Definition of the PurchaseOrde table
object PurchaseOrder extends Table[PurchaseOrde]("PURCHASE_ORDER") {
  ...
  // def customerIdEntity = foreignKey("FOREIGNKEY_CUSTOMER_ID", customerId, Customer)(_.id)
  
  def productId = column[Int]("PRODUCT_ID") 
  // def productIdEntity = foreignKey("FOREIGNKEY_PRODUCT_ID", productId, Product)(_.id)
  ...
  def * = orderNum.? ~ customerId ~ productId ~ quantity ~ shippingCost ~ salesDate ~ shippingDate ~ freightCompany <> (PurchaseOrde.apply _, PurchaseOrde.unapply _)
  ...
  def list(page: Int = 0, pageSize: Int = 10, orderBy: Int = 1, order: String, asc: Boolean = false, filter: String = "%") = {
    val members = ru.typeOf[PurchaseOrde].members.filter(m => m.isTerm && !m.isMethod).toList

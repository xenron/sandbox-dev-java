...
case class MicroMarke(zipCode : Option[String], radius : Option[Int], areaLength : Option[Int], areaWidth : Option[Int])

// Definition of the MicroMarke table
object MicroMarket extends Table[MicroMarke]("MICRO_MARKET") {
  ...
  def * = zipCode.? ~ radius ~ areaLength ~ areaWidth <> (MicroMarke.apply _, MicroMarke.unapply _)
  //def autoInc = id.? ~ zipCode.? ~ radius ~ areaLength ~ areaWidth <> (MicroMarke.apply _, MicroMarke.unapply _) returning id 
  ...
  def list(page: Int = 0, pageSize: Int = 10, orderBy: Int = 1, order: String, asc: Boolean = false, filter: String = "%") = {
    val members = ru.typeOf[MicroMarke].members.filter(m => m.isTerm && !m.isMethod).toList
